...
void parse(int depth)
{
	...
	while (ip < v.size() - 1)
	{
		...
		if (hasThen(v[ip]))
		{
			...
			fprintf(fp, "res += \"%s\\n\";\n", stmt.c_str());
			fprintf(fp, "rule += \"If %s is %s\\n Then: \";", cond1.c_str(), cond2.c_str());
			fprintf(fp, "rule += \"%s\\n\";}\n", stmt.c_str());
		}
		else
		{
			...
			cond2 = getCond2ex(v[tip]);
			fprintf(fp, "rule += \"If %s is %s \";", cond1.c_str(), cond2.c_str());
			tip++;
			while (!hasThen(v[tip]))
			{
				cond1 = getCond1(v[tip]);
				cond2 = getCond2ex(v[tip]);
				fprintf(fp, "rule += \"and %s == %s\";", cond1.c_str(), cond2.c_str());
				tip++;
			}
			cond1 = getCond1(v[tip]);
			cond2 = getCond2(v[tip]);
			stmt = getStmt(v[tip]);
			fprintf(fp, "rule += \"and %s == %s\\nThen: \";", cond1.c_str(), cond2.c_str());
			fprintf(fp, "rule += \"%s\\n\";\n", stmt.c_str());
